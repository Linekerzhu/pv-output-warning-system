    ...
    while current <= end:
        for hh in range(24):
            for mm in [0, 10, 20, 30, 40, 50]:
                target = current.replace(hour=hh, minute=mm)
                beijing_h = (hh + 8) % 24
                beijing_m = mm
                ...
                try:
                    ...
                    # Only log hourly to reduce noise
                    if mm == 0:
                        ...
                        print(f"  {current.strftime('%m/%d')} {beijing_h:02d}:00  {tag}  ({total_success} ok / {total_fail} fail)")
...
if __name__ == "__main__":
    ...
    print(f"Backfilling satellite GHI (10min): {start} to {end}")
